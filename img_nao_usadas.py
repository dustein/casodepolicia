...
def listar_imagens_usadas(pasta):
    """
    Analisa todos os arquivos HTML na pasta e subpastas e extrai as imagens
    que estão sendo referenciadas na pasta uploads (com diferentes padrões)
    """
    imagens_usadas = set()
    ...
    for root, dirs, files in os.walk(pasta):
        for arquivo in files:
            if arquivo.endswith('.html'):
                ...
                caminho_relativo = os.path.relpath(caminho_arquivo, pasta)
                print(f"\n📄 Analisando: {caminho_relativo}")
                ...
                try:
                    ...
                    print(f"   ⚠️ Erro ao processar arquivo {caminho_relativo}: {e}")
...
def listar_imagens_da_pasta_uploads(pasta_uploads):
    """
    Lista todos os arquivos de imagem na pasta uploads e suas subpastas
    """
    extensoes_imagem = {'.jpg', '.jpeg', '.png', '.gif', '.bmp', '.webp', '.svg', '.ico', '.tiff', '.tif'}
    imagens = set()
    ...
    # Dicionário para mapear nome do arquivo para caminho completo
    mapa_imagens = {}
    
    # Percorre recursivamente a pasta uploads
    for root, dirs, files in os.walk(pasta_uploads):
        for arquivo in files:
            ...
            if extensao in extensoes_imagem:
                caminho_relativo = os.path.relpath(caminho_completo, pasta_uploads)
                imagens.add(arquivo)
                mapa_imagens[arquivo] = caminho_completo
                print(f"   📸 Imagem encontrada: {caminho_relativo}")
    
    print(f"📊 Total de imagens na pasta uploads: {len(imagens)}")
    return imagens, mapa_imagens
...
def deletar_imagens_nao_usadas(mapa_imagens, imagens_nao_usadas):
    """
    Deleta as imagens não utilizadas usando o mapa de caminhos completos
    """
    imagens_deletadas = []
    erros_delecao = []
    ...
    for imagem in imagens_nao_usadas:
        if imagem in mapa_imagens:
            caminho_imagem = mapa_imagens[imagem]
            try:
                # Verifica se o arquivo ainda existe
                if os.path.exists(caminho_imagem):
                    os.remove(caminho_imagem)
                    imagens_deletadas.append(imagem)
                    print(f"   ✅ Deletada: {imagem} ({caminho_imagem})")
                else:
                    print(f"   ⚠️  Arquivo não encontrado: {imagem}")
            except Exception as e:
                erros_delecao.append((imagem, str(e)))
            ...
            print(f"   ⚠️  Caminho não encontrado para: {imagem}")
...
def main():
    ...
    print(f"🔍 Analisando arquivos HTML em: {pasta_atual} (incluindo subpastas)")
    print(f"📂 Verificando imagens em: {pasta_uploads} (incluindo subpastas)")
    print("=" * 60)
    ...
    # Lista as imagens na pasta uploads
    imagens_na_pasta, mapa_imagens = listar_imagens_da_pasta_uploads(pasta_uploads)
    ...
    if imagens_nao_usadas:
        print(f"🗑️  Encontradas {len(imagens_nao_usadas)} imagens NÃO UTILIZADAS:")
        for imagem in sorted(imagens_nao_usadas):
            caminho = mapa_imagens.get(imagem, "Caminho não encontrado")
            caminho_relativo = os.path.relpath(caminho, pasta_atual) if caminho != "Caminho não encontrado" else caminho
            print(f"   • {imagem} ({caminho_relativo})")
    else:
        print("✅ Todas as imagens na pasta uploads estão sendo utilizadas!")
        return []
    ...
    # Solicita confirmação e deleta as imagens
    if confirmar_delecao(imagens_nao_usadas):
        imagens_deletadas, erros = deletar_imagens_nao_usadas(mapa_imagens, imagens_nao_usadas)
        ...
        with open('imagens_nao_utilizadas.txt', 'w', encoding='utf-8') as f:
            ...
            for imagem in sorted(imagens_nao_usadas):
                caminho = mapa_imagens.get(imagem, "Caminho não encontrado")
                caminho_relativo = os.path.relpath(caminho, os.path.dirname(os.path.abspath(__file__))) if caminho != "Caminho não encontrado" else caminho
                f.write(f"{imagem} ({caminho_relativo})\n")
